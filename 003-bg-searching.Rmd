# Background: Information Searching {#ch-bg-search}
...
## Terminology {#sec-bg-search-terminology}
...
<!----------- fig:wilson-info-behaviour ----------->
```{r wilson-info-behaviour, fig.scap='(ref:scap-wilson-info-behaviour)', fig.cap = '(ref:cap-wilson-info-behaviour)',  echo=FALSE, out.width='50%', fig.align='center'}
nbShowFig("figs/wilson-info-behaviour")
```
<!-- each text reference needs to be their own single paragraph ! -->
(ref:cap-wilson-info-behaviour) Nested model of information behaviour by @wilson1999models.

(ref:scap-wilson-info-behaviour) Nested model of information behaviour by @wilson1999models.

<!---- fig:wilson-info-behaviour (leave a blank line before this) ------>


People's behaviour around information can be modelled as a nested Venn
diagram as proposed by @wilson1999models (Figure
\@ref(fig:wilson-info-behaviour)). **Information behaviour** is
...
## Three-stage Interactions with Online Search Systems {#sec-bg-search-3-stage}

<!----------- fig:info-search-models ----------->
```{r info-search-models, fig.scap='(ref:scap-info-search-models)', fig.cap = '(ref:cap-info-search-models)',  echo=FALSE, out.width='100%', fig.align='center'}
nbShowFig("figs/info-search-models")
```
<!-- each text reference needs to be their own single paragraph ! -->
(ref:cap-info-search-models) Models of information search process, with our coloured annotations identifying the three stages: (1) query formulation, (2) list-item selection, and (3) item examination.

(ref:scap-info-search-models) Models of information search process.

<!---- fig:info-search-models (leave a blank line before this) ------>
...
1. *Stage 1:* search query (re)formulation
2. *Stage 2:* list-item selection: search results evaluation (aka source selection)
3. *Stage 3:* item examination: content page evaluation (aka interacting with sources)
...
### Stage 1: Query (Re)formulation {#sec-bg-search-query}
...
<!----------- fig:res-Q-QRT-txnmy ----------->
```{r res-Q-QRT-txnmy, fig.scap='(ref:scap-res-Q-QRT-txnmy)', fig.cap = '(ref:cap-res-Q-QRT-txnmy)',  echo=FALSE, out.width='100%', fig.align='center'}
nbShowFig("figs/res-Q-QRT-txnmy")
```
<!-- each text reference needs to be their own single paragraph ! -->
(ref:cap-res-Q-QRT-txnmy) Comparison of Query Reformulation Types (QRTs) proposed by @boldi2009dango and @liu2010analysis.

(ref:scap-res-Q-QRT-txnmy) Comparison of Query Reformulation Types (QRTs) proposed in the literature.

<!---- fig:res-Q-QRT-txnmy (leave a blank line before this) ------>
...
ones ^[
    measured using Leacock-Chodorow semantic similarity metric [@leacock1998combining]
] instead. These observations can possibly be explained by @133's
...
<!----------- fig:int-Q ----------->
```{r int-Q, fig.scap='(ref:scap-int-Q)', fig.cap = '(ref:cap-int-Q)',  echo=FALSE, out.width='100%', fig.align='center'}
nbShowFig("figs/int-Q")
```
<!-- each text reference needs to be their own single paragraph ! -->
(ref:cap-int-Q) Investigating user-interactions with queries: **(a)** Visualizing the distribution of retrieved search results prior to running a query, for helping searchers understand their queries' effectiveness [@121]. The visualization is a stacked column chart with ten columns. Each column represents ten search results: first column represents results ranked 1-10, second column represents results 11-20, etc. Individual columns have three divisions, indicating the counts of results that: are already seen by the searcher (dark blue, top), will be re-retrieved, but have not been seen by the searcher (medium blue, middle), and will be newly-retrieved (bright teal blue, bottom). The system evaluates the searcher's query continuously as it is being typed, and updates the visualization in real-time. **(b)** Interfaces for examining interactions with query auto-completion (QAC), by ***(i)*** @129, and ***(ii)*** @125 (overlaid with heatmaps of eye fixations for all participants). This figure is best viewed in colour.

(ref:scap-int-Q) Investigating user-interactions with queries.

<!---- fig:int-Q (leave a blank line before this) ------>
...
<!----------- fig:res-Q-QAC-profiles ----------->

```{r res-Q-QAC-profiles, fig.scap='(ref:scap-res-Q-QAC-profiles)', fig.cap = '(ref:cap-res-Q-QAC-profiles)',  echo=FALSE, out.width='100%', fig.align='center'}
nbShowFig("figs/res-Q-QAC-profiles")
```
<!-- each text reference needs to be their own line / paragraph ! -->
(ref:cap-res-Q-QAC-profiles) Comparison of User behaviour profiles identified around Query Auto-Completion (QAC), from eye-tracking data, by @125 and @129.

(ref:scap-res-Q-QAC-profiles) Comparison of User behaviour profiles identified around Query Auto-Completion from eye-tracking data.

<!----------- fig:res-Q-QAC-profiles (leave a blank line before this) ----------->
...
### Stage 2: Search Results Evaluation / List-Item Selection {#sec-bg-search-list}
...
<!----------- fig:int-L-serp ----------->
```{r int-L-serp, fig.scap='(ref:scap-int-L-serp)', fig.cap = '(ref:cap-int-L-serp)',  echo=FALSE, out.width='100%', fig.align='center'}
nbShowFig("figs/int-L-serp")
```
<!-- each text reference needs to be their own single paragraph ! -->
(ref:cap-int-L-serp) Example interfaces for studying user-interactions with a search-engine results page (SERP): **(a)** a simplified SERP without query input facility, to judge relevance of search results (on a 4-level scale) for pre-determined search queries (in this case 'why do airplanes have differently shaped wings?'), from @63; **(b)** eye-tracking heatmap on an organic SERP from @115 [@117], showing the F-shaped pattern of visual attention; **(c)** a multilingual SERP from @132. This figure is best viewed in colour.

(ref:scap-int-L-serp) Interfaces for studying user-interactions with search-engine results.

<!---- fig:int-L-serp (leave a blank line before this) ------>
...
best viewed in colour. ](figs/int-L-serp.pdf){#fig:int-L-serp
width="\\linewidth"}
-->
...
#### Information Shown in Search Results (Surrogates)
<!--  \
...
<!----------- fig:res-L-serp-user-chars ----------->
```{r res-L-serp-user-chars, fig.scap='(ref:scap-res-L-serp-user-chars)', fig.cap = '(ref:cap-res-L-serp-user-chars)',  echo=FALSE, out.width='100%', fig.align='center'}
nbShowFig("figs/res-L-serp-user-chars")
```
<!-- each text reference needs to be their own single paragraph ! -->
(ref:cap-res-L-serp-user-chars) Effects of differences in user characteristics on interactions with SERPs: **(a)** exhaustive or *depth-first* user (User 1), vs. economic or *breadth-first* user (User 2), examining mostly irrelevant results in Task A, and mostly relevant results in Task B (both users followed the second link in Task B); vertical axis denotes vertical location on SERP, and horizontal axis denotes temporal ordering of result examination; from @102; (similar patterns were identified by @139, in the SERP examination behaviour of children) **(b)** children vs. adults examining SERPs from a German search engine for children (left), and Google (right); differently from adults, children exhaustively explored all search results, paid more attention to thumbnails and embedded media, and read less text-only snippets; from @124. Similar observations as with children were reported for searchers with dyslexia [@palani2020eye]. This figure is best viewed in colour.

(ref:scap-res-L-serp-user-chars) Differences in user characteristics on interactions with SERPs.

<!---- fig:res-L-serp-user-chars (leave a blank line before this) ------>

<!-- 
![ .
](figs/res-L-serp-user-chars.pdf){#fig:res-L-serp-user-chars
width="\\linewidth"}
-->
...
(Figure \@ref(fig:res-L-serp-user-chars)(a)). @117 demonstrated the use of
unsupervised clustering to re-identify the *economic*-*exhaustive* user
groups, based on differences in total fixation impact ^[
    a measure derived from eye fixation durations, proposed by @110
], scanpaths,
...
<!-- also talk about navigators vs explorers, and other such profiles -->
...
[@132] (Figure \@ref(fig:int-L-serp)(c)). We discussed in
...
<!----------- fig:int-L-serp-new-vertical ----------->
```{r int-L-serp-new-vertical, fig.scap='(ref:scap-int-L-serp-new-vertical)', fig.cap = '(ref:cap-int-L-serp-new-vertical)',  echo=FALSE, out.width='100%', fig.align='center'}
nbShowFig("figs/int-L-serp-new-vertical")
```
<!-- each text reference needs to be their own single paragraph ! -->
(ref:cap-int-L-serp-new-vertical) Google search engine result page (SERP) for the queries: **(a)** "coronavirus" **(b)** "toyota" **(c)** "evaporation", and **(d)** "life of pie". All screenshots are from 'above-the-fold', viewed on a $2560 \times 1440$ monitor. These examples highlight that modern SERPs have come a long way from a list of "ten blue links". SERPs are becoming consumable information-objects in their own right, and thus require different kinds of cognitive processing and interactions, than from the early days of the internet. Inspired and adapted from @yue2018optimizing. Accessed on May 5, 2020. This figure is best viewed in colour.

(ref:scap-int-L-serp-new-vertical) Examples of Google SERP going beyond the "ten blue links" paradigm.

<!---- fig:int-L-serp-new-vertical (leave a blank line before this) ------>
...
viewed in colour.
](figs/int-L-serp-new-vertical.pdf){#fig:int-L-serp-new-vertical
...
Balatsoukas & Ruthven [-@119;-@114] proposed a list of relevance criteria for understanding
...
(Figure \@ref(fig:int-L-serp-new-vertical)). @128 studied the influence of
...
### Stage 3: Content Page Evaluation / Item Examination {#sec-bg-search-content-page}
...
the observed "F-shaped-patterns" ^[
...
## Effects of Expertise and Working Memory on Search Behaviour {#sec-bg-search-expertise}

<!----------- fig:search-behaviours ----------->
```{r search-behaviours, fig.scap='(ref:scap-search-behaviours)', fig.cap = '(ref:cap-search-behaviours)',  echo=FALSE, out.width='100%', fig.align='center'}
nbShowFig("figs/search-behaviours")
```
<!-- each text reference needs to be their own single paragraph ! -->
(ref:cap-search-behaviours) Literature reviews by @rieh2016searching and @vakkari2016searching identified the following search behavioural traits as indicative of domain experts, or novices undergoing learning to become experts.

(ref:scap-search-behaviours) Components of metacognition.

<!---- fig:search-behaviours (leave a blank line before this) ------>

<!-- 
![image](figs/search-behaviours.pdf){width="\\linewidth"} 
-->
...
Previous work on domain knowledge and expertise have linked ^[and continue to link] domain
...
@urgo2022learning compare and contrast these assessment techniques extensively in their very comprehensive literature review.
...
discussed in Section \@ref(sec-bg-learn-artefact)). **Visual mapping** techniques such
...
### Longitudinal studies {#sec-bg-search-longitudinal-studies}
...
[@kelly2006measuringa; @kelly2006measuringb; @kuhlthau2004seeking; @wildemuth2004effects; @vakkari2001changes; @white2009characterizing].
...
the exception rather than the norm" [@koeman2020hciux]. "An
...
### Supporting sensemaking and reflection {#sec-bg-search-sensemaking}

As we saw in Section \@ref(sec-bg-learn-sensemaking), learning *is* sensemaking. Yet,
...
>
> `r nbQuoteAuthor('--- @ko2021seeking')` (emphasis our own)
>

## Summary {#sec-bg-search-summary}
