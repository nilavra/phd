...
## Study Design {#sec:method_exp_design}
...
reference="sec:bg_learn_artefact"}) will be built over time throughout
...
Appendix [\[ch:pilot_study\]](#ch:pilot_study){reference-type="ref"
reference="ch:pilot_study"}. The final project overview from the
...
[\[sec:bg_concept_maps\]](#sec:bg_concept_maps){reference-type="ref"
reference="sec:bg_concept_maps"}, concept maps are an appropriate way to
...
## Search Task Template {#sec:method_search_task_template}
...
[1.4](#sec:method_procedure){reference-type="ref"
reference="sec:method_procedure"}). The search tasks mentioned will
...
## Procedure {#sec:method_procedure}
...
### SUR1: Entry Survey {#sec:method_sur1}
...
3.  **Search and IT proficiency:** (Appendix
    [\[app:search_it_proficiency\]](#app:search_it_proficiency){reference-type="ref"
    reference="app:search_it_proficiency"}) Captures previous search
...
4.  **Course Load and other engagements:** (Appendix
    [\[app:course_load\]](#app:course_load){reference-type="ref"
    reference="app:course_load"}) To determine how busy the participant
...
5.  **Note-taking Strategies:** (Appendix
    [\[app:note_taking_strategies\]](#app:note_taking_strategies){reference-type="ref"
    reference="app:note_taking_strategies"}) Captures styles and
...
7.  **Self-regulation:** (Appendix
    ...
    [\[sec:bg_learn_self_regulation\]](#sec:bg_learn_self_regulation){reference-type="ref"
    reference="sec:bg_learn_self_regulation"}).
...
8.  **Metacognition:** (Appendix
    ...
    [\[sec:bg_learn_metacognition\]](#sec:bg_learn_metacognition){reference-type="ref"
    reference="sec:bg_learn_metacognition"}).
...
2.  **Two Search Tasks:** Participants will perform two search tasks, of
    ...
    are in Appendix
    [\[ch:pilot_study\]](#ch:pilot_study){reference-type="ref"
    reference="ch:pilot_study"}. Each search task is expected to take
    around 20 minutes.
    ...
    [\[app:pre_task_qsn\]](#app:pre_task_qsn){reference-type="ref"
    reference="app:pre_task_qsn"}), which asks participants to self-rate
    ...
    [\[app:post_task_qsn\]](#app:post_task_qsn){reference-type="ref"
    reference="app:post_task_qsn"}) asks participants to self-rate their
    ...
    [\[app:cta_v\_silent\]](#app:cta_v_silent){reference-type="ref"
    reference="app:cta_v_silent"}).
...
### SUR2: Mid-Term Survey {#sec:method_sur2}
...
\[SUR1\] (Section [1.4.1](#sec:method_sur1){reference-type="ref"
reference="sec:method_sur1"}), with two modifications. First, the
...
[\[app:midterm_survey\]](#app:midterm_survey){reference-type="ref"
reference="app:midterm_survey"}). Participants will be compensated with
...
[\[ch:pilot_study\]](#ch:pilot_study){reference-type="ref"
reference="ch:pilot_study"}) are presented in Appendix
[\[app:post_task_interview\]](#app:post_task_interview){reference-type="ref"
reference="app:post_task_interview"}, which can be reused.
...
survey \[SUR2\] (Section [1.4.4](#sec:method_sur2){reference-type="ref"
reference="sec:method_sur2"}), with the Intrinsic Motivation Inventory
(IMI) reworded to reflect the end-point of the semester (Appendix
[\[app:final_survey\]](#app:final_survey){reference-type="ref"
reference="app:final_survey"}). Participants will be compensated with
...
[\[sec:bg_search_3\_stage\]](#sec:bg_search_3_stage){reference-type="ref"
reference="sec:bg_search_3_stage"}. These measures include query
...
[\[sec:app_vars_qry\]](#sec:app_vars_qry){reference-type="ref"
reference="sec:app_vars_qry"} through
[\[sec:app_vars_overall_search\]](#sec:app_vars_overall_search){reference-type="ref"
reference="sec:app_vars_overall_search"}.
...
[\[sec:app_vars_concept_maps\]](#sec:app_vars_concept_maps){reference-type="ref"
reference="sec:app_vars_concept_maps"}); *(ii)* analysis of written
...
present in Appendix
[\[sec:app_pilot_ses3\]](#sec:app_pilot_ses3){reference-type="ref"
reference="sec:app_pilot_ses3"} Task 3).
