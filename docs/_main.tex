...
\section{Latent Profile Analysis}\label{latent-profile-analysis-1}}
...
\hypertarget{query-formulation}{%
\section{Query Formulation}\label{query-formulation}}
...
For all tasks, on average, the high group issued longer queries than the low group (Figure ResP4090733126835336320), except before the Outline (\texttt{PHASE2B}), and the Final Paper submission (\texttt{PHASE2D}).
...
The total query length for the low group increased monotonically over the semester during \texttt{PHASE2} (Figure ResP494716060171793945), with the longest queries being issued by this group before the Final Paper submission phase (\texttt{PHASE2D}).
...
\hypertarget{count-of-queries-per-search-task}{%
\subsection{Count of queries per search task}\label{count-of-queries-per-search-task}}
...
The difference in the total number of queries issued by the two groups approached statistical significance according to a two-tailed Mann-Whitney U test, but the effect size was very weak \((U = 14.0, n_{low}=8, n_{high}=8, p=.06, CLES=0.22)\).

\hypertarget{count-of-reformulated-query-types}{%
\subsection{Count of reformulated query types}\label{count-of-reformulated-query-types}}
...
The low group issued more number of \textbf{New} queries in total (Figure ResN3873804091287075248) and on average (Figure ResP3026382842695489517), compared to the high group.
...
The low group demonstrated a significantly higher count of total \textbf{Query Generalizations} \((U = 44.5, n_{low}=7, n_{high}=7, p=.01, CLES=0.91)\).
...
The trend in the count of total \textbf{Query Specializations} was similar to that of Query Generalizations (Figure ResN6450294753032251158).
...
\textbf{Word Substitutions} were performed in all the tasks by the high group, whereas the low group skipped it during Proposal, and the \texttt{PHASE3-BIAS} task (Figure ResN3108315939720148502).
...
\subsection{Entropy of Query Reformulation}\label{sec-res-query-H}}
...
\hypertarget{transition-entropy-of-query-reformulation}{%
\subsubsection{Transition Entropy of Query Reformulation}\label{transition-entropy-of-query-reformulation}}
...
\hypertarget{stationary-entropy-of-query-qeformulation}{%
\subsubsection{Stationary Entropy of Query Qeformulation}\label{stationary-entropy-of-query-qeformulation}}
...
\hypertarget{webpages-visits}{%
\section{Webpages Visits}\label{webpages-visits}}
...
\hypertarget{l-lists-search-results}{%
\subsection{L: Lists / Search Results}\label{l-lists-search-results}}

\hypertarget{lweb}{%
\subsubsection{LWEB}\label{lweb}}
...
\hypertarget{i-information-objects-content-pages}{%
\subsection{I: Information Objects / Content Pages}\label{i-information-objects-content-pages}}

\hypertarget{iweb---normal-web-pages}{%
\subsubsection{IWEB - Normal web pages}\label{iweb---normal-web-pages}}
...
\subsubsection{IPUB - Publications}\label{ipub---publications}}

Count

Dwell Time

\hypertarget{entropy-of-search-tactics}{%
\section{Entropy of Search Tactics}\label{entropy-of-search-tactics}}
...
\hypertarget{transition-entropy-of-search-tactics}{%
\subsection{Transition Entropy of Search Tactics}\label{transition-entropy-of-search-tactics}}
...
\hypertarget{stationary-entropy-of-search-tactics}{%
\subsection{Stationary Entropy of Search Tactics}\label{stationary-entropy-of-search-tactics}}
...
\hypertarget{latent-profile-analysis-2}{%
\subsection{Latent Profile Analysis}\label{latent-profile-analysis-2}}
