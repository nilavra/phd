...
The average length of queries issued by the high group were always longer than those issued by the low group, except before the Outline (\texttt{PHASE2B}) and the Final Paper submission (\texttt{PHASE2D}) (Figure \ref{fig:ResP2136245316506143308}).
...
The total query length for the low group increased monotonically over the semester during \texttt{PHASE2} (Figure \ref{fig:ResP7744164942895533217}), with the longest queries being issued by this group before the Final Paper submission phase (\texttt{PHASE2D}).
...
The number of queries issued per task (Figure \ref{fig:ResN5999530166888685231}) followed a similar pattern as the total query length (Figure \ref{fig:ResP7744164942895533217}).
...
The difference in the number of queries issued by the two groups approached statistical significance according to a two-tailed Mann-Whitney U test \((U = 1021.5, p = .09, ES = 0.61)\), with a moderate effect size.
...
The low group issued more New queries, Generalizations, Specializations, and Repeat queries (combined with hub-and-spoke moves) than the high group.
On the other hand, the high group had more word substitutions in their queries than the low group.
The differences in New queries \((U = 1066.0, p = .03, ES = 0.63)\)
and Repeat queries \((U = 1094.5, p = .03, ES = 0.63)\)
...
\tightlist
\item
  ResP4286504130058349795 New
\item
  ResN1744315778481681652 Generalization
\item
  ResP2653175457120298457 Specialization
\item
  ResN2468268136429851311 Word Substitution
\item
  ResP5924714325943281540 Repeat
\end{itemize}

The low group had their highest counts of all query reformulation types (barring Repeat) while writing the final paper.
...
They had their highest count of new queries and query specializations during the outline phase, indicating that they may have been more proactive in identifying new avenues for research earlier in the semester.
They also had their highest count of query generalizations during the writing of the rough draft, which may suggest that they were better able to synthesize and generalize information from their sources at an earlier stage in the writing process.
...
The high group had fewer total clicks per query \((U = 1069.5, p = .03, ES = 0.64)\), and a lower average number of clicks per query \((U = 1194.0, p = .01, ES = 0.65)\), compared to the low group, across most of the tasks during the semester (Figure \ref{fig:ResP6921334635387427729}).
...
Additionally, the high group had lower variability (standard deviation) in the number of clicks per query compared to the low group \((U = 1235.0, p = .00, ES = 0.68)\), and all of these differences were statistically significant.
...
\hypertarget{dwell-time-on-search-results-pages}{%
\subsection{Dwell Time on Search Results Pages}\label{dwell-time-on-search-results-pages}}
...
\hypertarget{publication-search-results-digital-libraries}{%
\subsection{Publication Search Results / Digital Libraries}\label{publication-search-results-digital-libraries}}
...
\hypertarget{iweb---normal-web-pages}{%
\subsection{IWEB - Normal web pages}\label{iweb---normal-web-pages}}

Count

Dwell Time

\hypertarget{ipub---publications}{%
\subsection{IPUB - Publications}\label{ipub---publications}}

Count

Dwell Time

\hypertarget{iall---all-i-pages}{%
\subsection{IALL - All I pages}\label{iall---all-i-pages}}

\hypertarget{entropy-analyses-of-sequences}{%
\section{Entropy Analyses of Sequences}\label{entropy-analyses-of-sequences}}

\hypertarget{sec-res-query-H}{%
\subsection{Query Reformulation Sequences}\label{sec-res-query-H}}
...
The transition entropy of query reformulations showed interesting patterns between the low and high groups (Figure \ref{fig:ResP2794594806936084269}).
While the differences were not significant over the entire duration of the tasks, there was a significant difference in the middle one-third interval of each task \((U = 131.0, p = .01, ES = 0.28)\).
The low group showed a decrease in transition entropy from proposal to outline, then an increase from outline to rough draft, and finally, from rough draft to final paper.
On the other hand, the high group had the lowest transition entropy during the proposal, followed by a slight increase during the outline, another decrease during the rough draft, and then a maximum during the final paper.

The low group had the least randomness (transition entropy) during outline, while the high group had the most randomness at this stage.
Subsequently, the randomness in the high group decreased, whereas that in low group increased (Figure \ref{fig:ResP2794594806936084269}).
...
However, as the semester progressed, the low group's approach became more random, while the high group's approach became more structured.
These results indicate that the low group may have struggled to adapt their query reformulation strategies as they moved through the different stages of writing the paper, while the high group was more able to adjust their strategies and maintain structure in their approach.

The stationary entropy of query reformulations of the low and high groups varied over the different phases of the research paper writing process (Figure \ref{fig:ResP5913968108765420078}). The low group's stationary entropy remained almost stable throughout, while the high group's stationary entropy increased from proposal to outline and then decreased from rough draft to final paper. The differences were approaching significance for the entire task duration \((U = 721.5, p = .09, ES = 0.40)\), and were significant near the beginning of the task \((U = 472.0, p = .009, ES = 0.32)\), and during the middle of the tasks \((U = 148.0, p = .01, ES = 0.28)\).

Overall, the differences in entropies between the low and high groups indicate that these two groups had distinct information searching behaviours throughout the writing process.
The low group maintained a more consistent approach to their information searching behaviour, and thus preferred some query reformulation types more over others.
The high group's approach varied more depending on the stage of the writing process, and thus employing all the different types query reformulations uniformly (i.e.~with equal probabilities).
...
The subsequent decrease in stationary entropy from rough draft to final paper suggests that they were able to narrow down their focus and consolidate their understanding of the subject-matter.

\hypertarget{search-tactic-sequences}{%
\subsection{Search Tactic Sequences}\label{search-tactic-sequences}}
