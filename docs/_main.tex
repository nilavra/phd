...
\chapter{Data Analysis Framework}\label{data-analysis-framework}}
...
This chapter describes the general framework for analysing the data collected in the \emph{LongSAL} study.
...
\hypertarget{individual-differences-questionnaires-data-analysis-steps}{%
\section{Individual Differences Questionnaires Data Analysis Steps}\label{individual-differences-questionnaires-data-analysis-steps}}

\hypertarget{motivation-metacognition-and-self-regulation-data}{%
\subsection{Motivation, Metacognition, and Self Regulation Data}\label{motivation-metacognition-and-self-regulation-data}}

\hypertarget{latent-profile-analysis-lpa}{%
\subsection{Latent Profile Analysis (LPA)}\label{latent-profile-analysis-lpa}}
...
\hypertarget{yasbil-search-log-data-analysis-steps}{%
\section{YASBIL Search Log Data Analysis Steps}\label{yasbil-search-log-data-analysis-steps}}

\hypertarget{url-categorization}{%
\subsection{URL Categorization}\label{url-categorization}}
...
\tightlist
\item
  \texttt{L}: Search Result Pages, i.e., a \textbf{L}ist of Information Objects

  \begin{itemize}
  \tightlist
  \item
    \texttt{L.PUB}: Publication Search Results, e.g., on university library websites, digital libraries, Google Scholar, etc.
  \item
    \texttt{L.WEB}: Web Search Engine Result Pages (SERPs)
  \end{itemize}
\item
  \texttt{I}: Content pages, i.e., Individual \textbf{I}nformation Objects

  \begin{itemize}
  \tightlist
  \item
    \texttt{I.PUB}: Academic Publications
  \item
    \texttt{I.WEB}: Webpages that have the potential to provide relevant (academic) information for the search task, but are not publication. E.g., Wikipedia articles, relevant blog posts, government and non-profit websites, etc. Some of them were classified automatically (e.g., Wikipedia), while others were classified after manual inspection.
  \end{itemize}
\item
  \texttt{MISC}: URLs for webpages that did not fit in any of the above category
\end{itemize}
...
\hypertarget{active-tab-identification}{%
\subsection{Active Tab Identification}\label{active-tab-identification}}

\hypertarget{identifying-levels-of-search-activity}{%
\subsection{Identifying Levels of Search Activity}\label{identifying-levels-of-search-activity}}
...
\hypertarget{combining-individual-differences-with-search-logs}{%
\section{Combining Individual Differences with Search Logs}\label{combining-individual-differences-with-search-logs}}

\hypertarget{entropy-analyses-of-search-behaviour-sequences}{%
\section{Entropy Analyses of Search Behaviour Sequences}\label{entropy-analyses-of-search-behaviour-sequences}}
...
\tightlist
\item
  \texttt{QUERY}: issuing a search query
\item
  \texttt{CLICK}: mouse click
\item
  \texttt{IDLE}: participant stays idle for more than one minute (TODO CITATION)
\item
  \texttt{SESSION\_BREAK}: participant becomes idle for more than 30 minutes (Google Analytics defines a session break as 30 minutes of inactivity \footnote{\url{https://support.google.com/analytics/answer/2731565\#zippy=\%2Cin-this-article}})
\item
  \texttt{L.PUB}: visiting a publication search result page
\item
  \texttt{L.WEB}: visiting a web SERP
\item
  \texttt{I.PUB}: visiting a scholarly publication
\item
  \texttt{I.WEB}: visiting a non-scholarly content page
\item
  \texttt{TASKPAGE}: visiting webpages related to the study, i.e.~the Qualtrics questionnaires
\end{enumerate}
...
{\centering \includegraphics[width=1\linewidth]{figs/ResP4650362619704565728} 

}

\caption[Shortcaption.]{Longcaption.}\label{fig:ResP4650362619704565728}
\end{figure}
...
\hypertarget{results-from-phase2-longitudinal-searching-for-writing-research-paper}{%
\chapter{\texorpdfstring{Results from \texttt{PHASE2}: Longitudinal Searching for Writing Research Paper}{Results from PHASE2: Longitudinal Searching for Writing Research Paper}}\label{results-from-phase2-longitudinal-searching-for-writing-research-paper}}
...
\hypertarget{sec-res-lpa}{%
\section{Latent Profile Analysis}\label{sec-res-lpa}}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{figs/lpa-profile-model-57} 

}

\caption[Groups identified by Latent Profile Analysis.]{Mean Values of Indicator Variables for the two identified groups via Latent Profile Analysis (LPA). The grouping was based on the self-reported values of motivation (IMI), metacognition (MAI), and self-regulation (SRQ).}\label{fig:lpa-profile-model-57}
\end{figure}
...
\hypertarget{length-of-queries}{%
\subsection{Length of Queries}\label{length-of-queries}}

ResP3914747988879627169 \textbar{} ResN7152194718394476899
...
The average length of queries issued by the two groups was not very different during the Proposal (\texttt{PHASE2A}), Outline (\texttt{PHASE2B}), and Final Paper (\texttt{PHASE2D}) (Figure ResP3914747988879627169).
However, the difference in average query length between the two groups was almost significant during the Rough Draft stage (\texttt{PHASE2C}), with the low group issuing around 8 words per query, whereas the high group issuing about 4 words per query.

The total query length for the low group increased monotonically over the semester during (Figure \ref{fig:ResN7152194718394476899}), with the longest queries being issued by this group before the Final Paper submission phase (\texttt{PHASE2D}).
On the other hand, queries from the high group were the longest during the Outline phase (\texttt{PHASE2B}), and then it decreased in the subsequent phases.
...
A quote from participant \texttt{P006\_HANOI} illustrates this phenomenon:

\begin{quote}
\emph{``so far for this rough draft I haven't actually had to google anything since I downloaded everything into pdfs during the outline stage and haven't needed any new material so far.''}
...
We faced a challenge in disentangling \textbf{Repeat} queries from ``hub-and-spoke'' behaviour, where the user goes back and forth between the SERP and a content page by using the browser's forward and back buttons \footnote{The SERP is the hub, which represents the initial point of inquiry, while the spokes represent the subsequent branches of exploration along different content pages. This search behaviour is often used when users have a general idea of the topic they are interested in, but need to explore different facets of the topic to narrow down their search and find relevant information.}.
...
\hypertarget{sec-res-query-H}{%
\subsection{Entropy of Query Reformulation Types}\label{sec-res-query-H}}
...
Values of stationary entropy vary between 0 and \((\log s)\), where \(s\) is the number of possible states (QRTs)\footnote{The explanation is adapted from He et al. (\protect\hyperlink{ref-he2016beyond}{2016})}.
...
\hypertarget{l-interaction-with-search-results-source-selection-item-selection}{%
\section{L: Interaction with Search Results / Source Selection / Item Selection}\label{l-interaction-with-search-results-source-selection-item-selection}}

\hypertarget{number-of-clicks-per-query}{%
\subsection{Number of Clicks per Query}\label{number-of-clicks-per-query}}

ResN6629066624252186314 \textbar{} ResP5466996162614873067 \textbar{} ResP1250472196403953856
...
\hypertarget{dwell-time-on-search-results}{%
\subsection{Dwell Time on Search Results}\label{dwell-time-on-search-results}}

\hypertarget{publication-search-results}{%
\subsubsection{Publication Search Results}\label{publication-search-results}}

ResP8346620135317238453 \textbar{} ResN4762872426347949231
...
\hypertarget{web-search-results-serps}{%
\subsubsection{Web Search Results (SERPs)}\label{web-search-results-serps}}

ResP2704210890287754729 \textbar{} ResN2852099729940598719
...
\hypertarget{i-interaction-with-sources-content-pages-information-objects}{%
\section{I: Interaction with Sources / Content Pages / Information Objects}\label{i-interaction-with-sources-content-pages-information-objects}}

\hypertarget{academic-publications}{%
\subsection{Academic Publications}\label{academic-publications}}
...
\hypertarget{non-scholarly-normal-content-pages}{%
\subsection{Non-scholarly / Normal Content pages}\label{non-scholarly-normal-content-pages}}
...
\hypertarget{results-from-phase1-and-phase3-repeated-vs-new-search-tasks}{%
\chapter{\texorpdfstring{Results from \texttt{PHASE1} and \texttt{PHASE3}: Repeated vs New Search Tasks}{Results from PHASE1 and PHASE3: Repeated vs New Search Tasks}}\label{results-from-phase1-and-phase3-repeated-vs-new-search-tasks}}
...
\hypertarget{querying-behaviour}{%
\section{Querying Behaviour}\label{querying-behaviour}}

\hypertarget{length-of-queries-1}{%
\subsection{Length of Queries}\label{length-of-queries-1}}

\hypertarget{number-of-queries-per-search-task-1}{%
\subsection{Number of Queries per Search Task}\label{number-of-queries-per-search-task-1}}

\hypertarget{number-of-clicks-per-query-1}{%
\subsection{Number of Clicks per Query}\label{number-of-clicks-per-query-1}}
...
\hypertarget{sec-res-query-H}{%
\section{Entropy Analysis of Querying Behaviour}\label{sec-res-query-H}}
...
\hypertarget{app-plots-query}{%
\chapter{Supplementary Plots - Query Reformulation}\label{app-plots-query}}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{figs/ResN5894660104957913932} 

}

\caption[Query Length (Total) -- All Task(s).]{Query Length (Total) -- All Task(s).}\label{fig:ResN5894660104957913932}
\end{figure}
...
{\centering \includegraphics[width=1\linewidth]{figs/ResP6726131652814458688} 

}

\caption[Query Length (Average) -- All Task(s).]{Query Length (Average) -- All Task(s).}\label{fig:ResP6726131652814458688}
\end{figure}
...
{\centering \includegraphics[width=1\linewidth]{figs/ResP7469411730579087048} 

}

\caption[Number of Queries per Search Task (Total) -- All Task(s).]{Number of Queries per Search Task (Total) -- All Task(s).}\label{fig:ResP7469411730579087048}
\end{figure}
...
{\centering \includegraphics[width=1\linewidth]{figs/ResN8316003849035045794} 

}

\caption[Number of New Queries (Average) -- All Task(s).]{Number of New Queries (Average) -- All Task(s).}\label{fig:ResN8316003849035045794}
\end{figure}
...
{\centering \includegraphics[width=1\linewidth]{figs/ResN3604624796642370520} 

}

\caption[Number of Query Generalizations (Average) -- All Task(s).]{Number of Query Generalizations (Average) -- All Task(s).}\label{fig:ResN3604624796642370520}
\end{figure}
...
{\centering \includegraphics[width=1\linewidth]{figs/ResP5443351906342031714} 

}

\caption[Number of Query Specializations (Average) -- All Task(s).]{Number of Query Specializations (Average) -- All Task(s).}\label{fig:ResP5443351906342031714}
\end{figure}
...
{\centering \includegraphics[width=1\linewidth]{figs/ResN4290201084087196149} 

}

\caption[Number of Word Substitutions in Queries (Average) -- All Task(s).]{Number of Word Substitutions in Queries (Average) -- All Task(s).}\label{fig:ResN4290201084087196149}
\end{figure}
...
{\centering \includegraphics[width=1\linewidth]{figs/ResN9056495301645634830} 

}

\caption[Number of Repeat Queries (Average) -- All Task(s).]{Number of Repeat Queries (Average) -- All Task(s).}\label{fig:ResN9056495301645634830}
\end{figure}
...
{\centering \includegraphics[width=1\linewidth]{figs/ResN2968292861275564559} 

}

\caption[Stationary Entropy of Query Reformulations for Whole Task -- All Task(s).]{Stationary Entropy of Query Reformulations for Whole Task -- All Task(s).}\label{fig:ResN2968292861275564559}
\end{figure}
...
{\centering \includegraphics[width=1\linewidth]{figs/ResN634943316862196398} 

}

\caption[Stationary Entropy of Query Reformulations at Task Beginning -- All Task(s).]{Stationary Entropy of Query Reformulations at Task Beginning -- All Task(s).}\label{fig:ResN634943316862196398}
\end{figure}
...
{\centering \includegraphics[width=1\linewidth]{figs/ResN6777243644016606483} 

}

\caption[Stationary Entropy of Query Reformulations at Task Middle -- All Task(s).]{Stationary Entropy of Query Reformulations at Task Middle -- All Task(s).}\label{fig:ResN6777243644016606483}
\end{figure}
...
{\centering \includegraphics[width=1\linewidth]{figs/ResN6573661794052961083} 

}

\caption[Stationary Entropy of Query Reformulations at Task End -- All Task(s).]{Stationary Entropy of Query Reformulations at Task End -- All Task(s).}\label{fig:ResN6573661794052961083}
\end{figure}
...
{\centering \includegraphics[width=1\linewidth]{figs/ResP878771263343515297} 

}

\caption[Number of Clicks per Query (Average) -- All Task(s).]{Number of Clicks per Query (Average) -- All Task(s).}\label{fig:ResP878771263343515297}
\end{figure}
...
{\centering \includegraphics[width=1\linewidth]{figs/ResN2805268620307804410} 

}

\caption[Number of Clicks per Query (Total) -- All Task(s).]{Number of Clicks per Query (Total) -- All Task(s).}\label{fig:ResN2805268620307804410}
\end{figure}
...
{\centering \includegraphics[width=1\linewidth]{figs/ResN5118140679486679749} 

}

\caption[Number of Clicks per Query (SD) -- All Task(s).]{Number of Clicks per Query (SD) -- All Task(s).}\label{fig:ResN5118140679486679749}
\end{figure}
...
\hypertarget{app-plots-search}{%
\chapter{Supplementary Plots - Search Behaviour}\label{app-plots-search}}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{figs/ResP5183239600404564866} 

}

\caption[Stationary Entropy of Search Tactics at Task Middle -- All Task(s).]{Stationary Entropy of Search Tactics at Task Middle -- All Task(s).}\label{fig:ResP5183239600404564866}
\end{figure}
